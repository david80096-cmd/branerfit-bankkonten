...
app.get("/api/storage-usage", async (req, res) => {
  console.log("[API] GET /api/storage-usage");
  try {
    const currentMonth = new Date().toISOString().slice(0, 7);
    const usage = db.prepare("SELECT SUM(size) as total FROM upload_history WHERE strftime('%Y-%m', timestamp) = ?").get(currentMonth) as { total: number | null } | undefined;
    const response = { 
      used: usage?.total || 0,
      limit: 10 * 1024 * 1024 * 1024 // 10GB
    };
    console.log("[API] Storage usage response:", response);
    res.json(response);
  } catch (error: any) {
    console.error("[API ERROR] /api/storage-usage:", error);
    res.status(500).json({ error: error.message });
  }
});

app.get("/api/storage-history", async (req, res) => {
  console.log("[API] GET /api/storage-history");
  try {
    const history = db.prepare(`
      ...
      LIMIT 12
    `).all();
    console.log("[API] Storage history response length:", history.length);
    res.json(history);
  } catch (error: any) {
    console.error("[API ERROR] /api/storage-history:", error);
    res.status(500).json({ error: error.message });
  }
});
...
app.post("/api/video/delete", async (req, res) => {
  try {
    if (!r2Config || !bucketName) {
    ...
    console.log("[R2 DELETE] Attempting to delete key:", key);
    ...
    console.log("File berhasil dihapus dari R2. Result:", JSON.stringify(result));

    // Hapus metadata dari database
    const dbResult = db.prepare("DELETE FROM video_metadata WHERE video_key = ?").run(key);
    console.log("Database cleanup result:", dbResult.changes, "rows deleted");
...
app.patch("/api/video", async (req, res) => {
  try {
    ...
    if (!oldKey || !newName) {
      return res.status(400).json({ error: "Invalid request: key and newName are required" });
    }
